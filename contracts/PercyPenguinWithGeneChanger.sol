...
import "./PercyPenguin.sol";
import "./IPercyPenguinWithGeneChanger.sol";


contract PercyPenguinWithGeneChanger is IPercyPenguinWithGeneChanger, PercyPenguin {
    ...
    event RandomizeGenomePriceChanged(uint256 newRandomizeGenomePrice);

    constructor(string memory name, string memory symbol, string memory baseURI, address payable _daoAddress, uint256 premintedTokensCount, uint256 _baseGenomeChangePrice, uint256 _percyPenguinPrice, uint256 totalSupply, uint256 _randomizeGenomePrice, uint256 _bulkBuyLimit, string memory _arweaveAssetsJSON) PercyPenguin(name, symbol, baseURI, _daoAddress, premintedTokensCount, _percyPenguinPrice, totalSupply, _bulkBuyLimit, _arweaveAssetsJSON) {
    ...
    function morphGene(uint256 tokenId, uint256 genePosition) public payable virtual override nonReentrant {
        ...
        uint256 excessAmount = msg.sender.sub(price);
        if (excessAmount > 0) {
        ...
        emit TokenMorphed(tokenId, oldGene, oldGene, price, PercyPenguinEventType.MORPH);
    }
    ...
    function randomizeGenome(uint256 tokenId) public payable override virtual nonReentrant {
        ...
        if (excessAmount > 0) {
            (bool returnExcessStatus, ) = _msgSender().call{value: excessAmount}("");
            require(returnExcessStatus, "Failed to reurn excess.");
        }

        uint256 oldGene = _genes[tokenId];
        _genes[tokenId] = geneGenerator.random();
        _genomeChanges[tokenId] = 0;
        emit TokenMorphed(tokenId, oldGene, _genes[tokenId], randomizeGenomePrice, PercyPenguinEventType.MORPH);
    }
    ...
    function _beforeGenomeChange(uint256 tokenId) internal virtual {
        ...
        require(ownerOf(tokenId) == _msgSender(), "PercyPenguinWithGeneChanger: cannot change genome of a token that is not owned");
    }
